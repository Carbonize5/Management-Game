...
func start_gen_world_around_castle(max_range:float) -> void:
	gen_world_around_tile(castle_tile, max_range)

func gen_world_around_tile(active_node:Node3D, max_range:float) -> void:
	...
	for node:Node3D in neighbours:
		...
		var dist : float = node_game_world_pos.distance_to(castle_tile.position)
		if dist <= max_range and not node_game_world_pos in map_tiles:
			...
			gen_world_around_tile(tile, max_range)
